...
def show_image_and_validation(blob_url, credit_card_info):
    ...
    if credit_card_info and credit_card_info["card_name"]:
        st.markdown(f"<h1 style= 'color: green;'>Cartao Válido</h1>", unsafe_allow_html=True)
        st.write(f"Nome do Titular: {credit_card_info['card_name']}")
        st.write(f"Banco do Emissor: {credit_card_info['bank_name']}")
        st.write(f"Data de Validade: {credit_card_info['expiry_date']}")
    else:
        st.markdown(f"<h1 style = 'color: red;'>Cartao Inválido</h1>", unsafe_allow_html=True)
        st.write("Este cartao nao eh valido.")
